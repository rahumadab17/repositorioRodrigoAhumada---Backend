const idCart = JSON.parse(localStorage.getItem('cart'))
const rutaFetch = 'http://localhost:8080/api/carts/'
document.getElementById('delCart').addEventListener('click', deleteCart)

getCartInfo()

function getCartInfo() {
    document.getElementById('cartID').innerText = `Cart ID ${idCart}`
    const targetDOM = document.getElementById('listaProductos')
    targetDOM.innerHTML = ''
    targetDOM.addEventListener('click', botonera)
    fetch(rutaFetch + idCart)
        .then(resp => resp.json())
        .then(data => {
            console.log(data)
            for (elem of data.cart) {
...
function botonera(e) {
    ...
    if (action === 'del') {
        // /:cid/producto/:pid
        const rutaDelete = rutaFetch + idCart + '/product/' + id
        ...
        const rutaUpdate = rutaFetch + idCart + '/product/' + id
